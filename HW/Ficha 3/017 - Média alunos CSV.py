...
#3) Dividir o loop pelo tamanho da lista

import csv #Para conseguir ler o texto em CSV
#1)
...
#Alternativa para o primeiro passo (Esse seria com utilização de arquivo de CSV)
notas = {} #Notas é um dicionário
soma = 0
with open(r"nota.csv", "r") as NotasEmTexto:
    reader = csv.reader(NotasEmTexto,lineterminator="\n") #cria um objeto de leitura do CSV
    count = 0
    for i in reader: #Para cada linha do CSV (usa o for loop para pegar os valores do objeto reader)
        notas[f"Nota do aluno {count+1}"] = i[1] #Adiciona a nota do aluno no dicionário notas
        count += 1
    
for i in range(1,50):
    soma += float(notas[f"Nota do aluno {i}"]) #Soma recebe o valor da nota no indice i da lista notas no primeiro indice (O indice 0 é o número do aluno (Se você utilizar o primeiro método isso não é necessário))
